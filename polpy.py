...
class Crypto(object):
	...
	def __init__(self, currencyPair,  startDate=False, interval=False, endDate = False):
		...
		if not startDate:
			startDate = time() - 1800 # month
		if not endDate:
			self.lastDate = time() 
		else:
			self.lastDate = endDate
		self.data = pd.DataFrame(polo.returnChartData(self.currencyPair, self.interval, startDate, self.lastDate)) #returnChartData(self, pair, period=False, start=False, end=False)
		# date high low open close volume quoteVolume weightedAverage+

	def update(self):
		currentDate = time()
		temp = pd.DataFrame(polo.returnChartData(self.currencyPair, self.interval, self.lastDate, currentDate))#self.data.append(pd.DataFrame(polo.returnChartData(self.currencyPair, self.interval, self.lastDate, currentDate)))
		self.lastDate = currentDate
		#self.data.append(self.data2)
		frames = [self.data, temp]
